...
> **Info** `젬`은 [rubygems.org](http://rubygems.org/)로부터 다운로드 받기 때문에 인터넷이 연결되지 않는경우 설치가 불가능하다. 그러나 이미 컴퓨터에 해당 `젬` 버전이 설치되어 있으면 인터넷 연결이 없이도 `bundle` 작업이 가능하다. 개인적으로 운영 중인 `젬` 저장소나 로컬 또는 원격 `git` 저장소로부터 직접 `젬`을 설치할 수도 있다.
